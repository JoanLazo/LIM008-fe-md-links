...
var isDirOrFile = function isDirOrFile(root) {
  var rootAbsolute = convertInAbsolute(root);
  var fileArray = [];

  if (fs.statSync(rootAbsolute).isDirectory() === false) {
    fileArray.push(rootAbsolute);
  } else {
    var readDirectory = fs.readdirSync(rootAbsolute);
    readDirectory.forEach(function (element) {
      var joinRoutes = path.join(rootAbsolute, element);
...
var readFilesMd = function readFilesMd(arrFiles) {
  var arrObjLinks = [];
  var arrRootFiles = isDirOrFile(arrFiles);
  arrRootFiles.forEach(function (file) {
...
var validateOption = function validateOption(arrFiles) {
  var arrObjLinks = readFilesMd(arrFiles);
  var newPromise = arrObjLinks.map(function (links) {
...
var totalLinks = function totalLinks(arrObjLinks) {
  var totalLinks = arrObjLinks.length;
  return totalLinks;
